...
public class XinziController {
    ...
    @Autowired
    private JintieService jintieService;//津贴
    @Autowired
    ...
    */
    ...
    public R save(@RequestBody XinziEntity xinzi, HttpServletRequest request){
        ...
        if(xinziEntity==null){
            ...
            YuangongEntity yuangong = yuangongService.selectById(xinzi.getYuangongId());
            if(yuangong == null)
                return R.error("查不到员工");
            DictionaryEntity one = dictionaryService.selectOne(new EntityWrapper<DictionaryEntity>()
            ...
            xinzi.setShifaJine(xinzi.getJixiaoJine()+xinzi.getJiangjinJine()+xinzi.getButieJine()+Double.valueOf(one.getBeizhu()));//薪资表特有,不需要可以手动删掉
            xinziService.insert(xinzi);
            return R.ok();
        }else {
            return R.error(511,"该员工该月份已有薪资记录,不能重复添加");
        }
    }
...
//                            xinziEntity.setJiangjinJine(data.get(0));                    //提成 要改的
//                            xinziEntity.setJixiaoJine(data.get(0));                    //绩效 要改的
//                            xinziEntity.setButieJine(data.get(0));                    //津贴 要改的
